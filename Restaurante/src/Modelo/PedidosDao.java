...
public class PedidosDao {
    ...
    public int RegistrarDetalle(DetallePedido det){
        ...
        String sql = "INSERT INTO detalle_pedidos (id_producto, precio, cantidad, id_pedido) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, det.getId_producto());  // Se reemplaza det.getNombre()
            ps.setDouble(2, det.getPrecio());
            ps.setInt(3, det.getCantidad());
            ps.setInt(4, det.getId_pedido());
            ps.execute();
        } catch (SQLException e) {
            System.out.println(e.toString());
        } finally{
    ...
    public Pedidos verPedido(int id_pedido){
        ...
        // Consulta que une con la tabla 'salas' para obtener el nombre de la sala
    ...
    public List finalizarPedido(int id_pedido){
       List<DetallePedido> Lista = new ArrayList<>();
       // AQUI SE HIZO UN CAMBIO similar a verPedidoDetalle: se usa el nuevo modelo sin 'nombre' ni 'comentario'
       String sql = "SELECT d.* FROM pedidos p INNER JOIN detalle_pedidos d ON p.id = d.id_pedido WHERE p.id = ?";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           ...
           while (rs.next()) {               
               DetallePedido det = new DetallePedido();
               det.setId(rs.getInt("id"));
               det.setId_producto(rs.getInt("id_producto")); // Se asigna el id_producto
               det.setPrecio(rs.getDouble("precio"));
               det.setCantidad(rs.getInt("cantidad"));
               // Se elimina la asignación de 'comentario'
               Lista.add(det);
           }
       } catch (SQLException e) {
           System.out.println(e.toString());
